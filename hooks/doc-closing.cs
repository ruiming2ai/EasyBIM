...
namespace EasyBIM.TempPhaseHooks
{
    public class TempPhaseDocumentClosingHook
    {
        ...
        {
            InvokeController("HandleDocumentClosing", sender, e, typeof(DocumentClosingEventArgs), "DocClosing");
        }
        ...
        private static Assembly FindControllerAssembly(object sender)
        {
            ...
            if (!string.IsNullOrWhiteSpace(expectedName))
            {
                ...
                {
                    Log("DocClosingHookAssemblyLoadFailed expectedAssembly=" + expectedName + " " + ExceptionText(ex));
                }
            }

            Assembly extensionModule = LoadControllerFromPath(FindControllerPath(version), "DocClosing");
            ...
            {
                Log(
                    "DocClosingHookWrongYearController"
