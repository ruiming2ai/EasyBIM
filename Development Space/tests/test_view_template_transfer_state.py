...
class ViewTemplateTransferStateTests(unittest.TestCase):
    ...
    def test_window_xaml_exposes_native_table_and_target_controls(self):
        ...
        self.assertIn('<DataGrid x:Name="settings_lv"', source)
        self.assertIn('AutoGenerateColumns="False"', source)
        self.assertIn('GridLinesVisibility="All"', source)
        self.assertIn('Text="Include"', source)
        self.assertIn('Text="Parameter"', source)
        self.assertNotIn('Text="Value"', source)
        self.assertNotIn('<ListView x:Name="settings_lv"', source)
        self.assertNotIn('SharedSizeGroup="IncludeColumn"', source)
        self.assertIn('Width="92"', source)
        self.assertIn('Width="*"', source)
        self.assertIn('Text="{Binding name}"', source)
        self.assertIn('IsChecked="{Binding is_selected, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"', source)
