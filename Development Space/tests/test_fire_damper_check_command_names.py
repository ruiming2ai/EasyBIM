"""Fire Damper Check's bundle, XAML wiring and the pins that keep it read-only."""
...
DAMPER_CHECK_DIR = REPO_ROOT / "EasyBIM.tab" / "Misc Tools.panel" / "Damper Check.pushbutton"
UI_MODULE = COMMAND_DIR / "fire_damper_check_ui.py"
STATE_MODULE = COMMAND_DIR / "fire_damper_check_state.py"
SETTINGS_MODULE = COMMAND_DIR / "fire_damper_check_settings.py"
LIB_DIR = REPO_ROOT / "lib" / "easybim"
REVIT_MODULE = COMMAND_DIR / "fire_damper_check_revit.py"
ENGINE_MODULE = LIB_DIR / "link_crossings.py"
SHARED_MODULES = ("duct_network_revit.py", "duct_network_state.py", "type_checklist.py",
                  "local_settings.py", "check_windows.py", "link_crossings.py")
...
WINDOW_CLASSES = {
    "FireDamperCheckSetupWindow.xaml": "SetupWindow",
    "FireDamperCheckResultsWindow.xaml": "ResultsWindow",
}
...
class FireDamperCheckBundleTests(unittest.TestCase):
    ...
    def test_expected_modules_exist(self):
        for name in ("script.py", "fire_damper_check_state.py", "fire_damper_check_settings.py",
                     "fire_damper_check_revit.py", "fire_damper_check_ui.py"):
    ...
    def test_the_readme_documents_the_tool(self):
        ...
        self.assertIn("## Fire Damper Check (Misc Tools)", readme)


class FireDamperCheckXamlTests(unittest.TestCase):
    ...
    def test_the_setup_window_carries_the_agreed_controls(self):
        names = _xaml_names(_xaml_root("FireDamperCheckSetupWindow.xaml"))
        required = {
            "ScopeModelRadio", "ScopeViewRadio", "DamperSearchBox", "DamperPanel",
            "DamperCountText", "DamperAllButton", "DamperNoneButton", "LinksPanel",
            "IncludeFloorsCheck", "NearBox", "HopsBox", "BandNextLevelRadio", "BandFixedRadio",
            "BandHeightBox", "StatusText", "CheckButton", "CancelButton",
        }
    ...
    def test_the_results_window_carries_what_the_shared_window_touches(self):
        names = _xaml_names(_xaml_root("FireDamperCheckResultsWindow.xaml"))
    ...
    def test_the_results_window_carries_the_agreed_controls(self):
        names = _xaml_names(_xaml_root("FireDamperCheckResultsWindow.xaml"))
        required = {
            "SearchBox", "NearBox", "NearDownButton", "NearUpButton", "HopsBox",
    ...
    def test_check_is_the_default_and_cancel_is_the_escape(self):
        source = (COMMAND_DIR / "FireDamperCheckSetupWindow.xaml").read_text(encoding="utf-8")
    ...
    def test_the_results_close_button_is_the_escape(self):
        source = (COMMAND_DIR / "FireDamperCheckResultsWindow.xaml").read_text(encoding="utf-8")
...
class FireDamperCheckIronPythonTests(unittest.TestCase):
    ...
    def test_ui_module_stays_free_of_revit_api(self):
        for path in (UI_MODULE, LIB_DIR / "check_windows.py"):
            ...
            self.assertNotIn("fire_damper_check_revit", source, path.name)
            self.assertNotIn("link_crossings", source, path.name)

    def test_the_revit_modules_never_open_a_transaction(self):
        for path in (REVIT_MODULE, ENGINE_MODULE):
            source = path.read_text(encoding="utf-8")
            self.assertNotIn("Transaction(", source, path.name)
            self.assertNotIn("TransactionGroup", source, path.name)
            self.assertNotIn("SubTransaction", source, path.name)
...
class FireDamperCheckLauncherTests(unittest.TestCase):
    ...
    def test_the_active_flag_differs_from_damper_checks(self):
        other = _module_constant(DAMPER_CHECK_DIR / "script.py", "ACTIVE_ENVVAR")
        self.assertNotEqual(_module_constant(SCRIPT, "ACTIVE_ENVVAR"), other)

    def test_the_bridge_is_created_inside_the_command_run(self):
        source = SCRIPT.read_text(encoding="utf-8")
        self.assertIn('external_events.ExternalEventBridge("EasyBIM Fire Damper Check")', source)
