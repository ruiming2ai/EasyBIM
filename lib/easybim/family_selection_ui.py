# -*- coding: utf-8 -*-
"""WPF windows for choosing loadable families - shared by Families Transfer
and Families Downgrade.

Page 1 (three source cards), the Revit-link page and the family browser
live here with their XAML under ``ui/``; each consumer passes its own title.
No Revit API or ``clr`` import belongs in this module.
"""
...
from easybim.family_selection_state import filter_family_options
from easybim.family_selection_state import filter_link_document_options
from easybim.family_selection_state import filter_open_family_document_options
from easybim.family_selection_state import filter_unchecked_options
from easybim.family_selection_state import format_selection_count_text
from easybim.family_selection_state import format_total_selection_text
...
from easybim.family_selection_state import get_selected_open_family_document_keys
from easybim.family_selection_state import group_family_options_by_category
from easybim.family_selection_state import needs_repopulate_after_bulk_toggle
...
from easybim.family_selection_state import restore_open_family_document_selection


_UI_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ui")
SOURCE_SELECTION_XAML = os.path.join(_UI_DIR, "family_selection_source.xaml")
FAMILY_SELECTION_XAML = os.path.join(_UI_DIR, "family_selection_families.xaml")
LINK_SELECTION_XAML = os.path.join(_UI_DIR, "family_selection_links.xaml")

# The default title; every consumer passes its own so the shared pages never
# name a tool they are not part of.
...
class SourceSelectionWindow(forms.WPFWindow):
    ...
    def __init__(
        ...
        self._title = _safe_text(title) or TITLE
        # The page names the tool that opened it; the XAML default is only
        # ever seen by a consumer that passes nothing.
        self.Title = self._title
        self.header_tb.Text = self._title
        subtitle = _safe_text(subtitle)
        if subtitle:
            self.subtitle_tb.Text = subtitle
...
class FamilySelectionWindow(forms.WPFWindow):
    ...
    def add_click(self, sender, args):
        ...
        if self.require_selection and not selected:
            forms.alert("Select at least one family or opened .rfa file.", title=self._title)
            return
...
class LinkSelectionWindow(forms.WPFWindow):
    ...
    def next_click(self, sender, args):
        ...
        if not self.selected_link_document_keys:
            forms.alert("Check at least one Revit link first.", title=self._title)
            return
        self.result = "next"
        self.Close()
