# -*- coding: utf-8 -*-
"""Auto-update helper for EasyBIM startup and manual reload."""
...
def classify_sync_result(before_state, after_state):
    ...
    if not is_repo_compliant(after_state):
        return {
            ...
            "message": "EasyBIM auto update did not end in a clean main state.",
        }

    if is_repo_compliant(before_state) and before_state.get("head") == after_state.get("head"):
        return {"status": STATUS_NO_OP, "message": "EasyBIM is already current."}

    if before_state.get("head") != after_state.get("head"):
        return {
            "status": STATUS_UPDATED,
            "message": "EasyBIM updated. pyRevit is reloading.",
        }
...
def _run_auto_update(trigger):
    ...
    if not os.path.isdir(repo_root):
        return _failure_result("EasyBIM auto update could not find the extension folder.", visible=True)

    git_command = resolve_git_command()
    if not git_command:
        return _failure_result("EasyBIM auto update could not find git on this machine.", visible=True)

    if not os.path.exists(os.path.join(repo_root, ".git")):
        return _failure_result("EasyBIM auto update requires a valid git checkout.", visible=True)
    ...
    if worktree_check["returncode"] != 0 or worktree_check["stdout"].strip().lower() != "true":
        return _failure_result("EasyBIM auto update requires a valid git checkout.", visible=True, details=worktree_check)
    ...
    if remote_check["returncode"] != 0 or not remote_check["stdout"].strip():
        return _failure_result("EasyBIM auto update requires an origin remote.", visible=True, details=remote_check)

    before_state = _read_repo_state(repo_root, git_command=git_command)
    if before_state["error"]:
        return _failure_result("EasyBIM auto update could not read the current repo state.", visible=True, details=before_state)
    ...
    if fetch_result["returncode"] != 0:
        ...
        return _failure_result(
            "EasyBIM auto update could not fetch origin/main.",
            visible=True,
            details=fetch_result,
        )
    ...
    if remote_head_result["returncode"] != 0:
        return _failure_result(
            "EasyBIM auto update could not resolve origin/main.",
            visible=True,
    ...
    if is_repo_compliant(before_state):
        return {"status": STATUS_NO_OP, "message": "EasyBIM is already current.", "visible": trigger == "manual"}
    ...
    if before_state.get("has_untracked", False):
        ...
        if preclean["returncode"] != 0:
            return _failure_result(
                "EasyBIM auto update could not remove untracked files.",
                visible=True,
                details=preclean,
            )
    ...
    if checkout_result["returncode"] != 0:
        return _failure_result(
            "EasyBIM auto update could not switch the extension repo to main.",
            visible=True,
    ...
    if reset_result["returncode"] != 0:
        return _failure_result(
            "EasyBIM auto update could not reset the extension repo to origin/main.",
            visible=True,
    ...
    if clean_result["returncode"] != 0:
        return _failure_result(
            "EasyBIM auto update could not clean untracked files.",
            visible=True,
    ...
    if after_state["error"]:
        return _failure_result("EasyBIM auto update could not read the final repo state.", visible=True, details=after_state)
...
def _reload_pyrevit(result):
    try:
        ...
        result["message"] = "EasyBIM updated, but pyRevit reload failed."
        _show_message(
            "EasyBIM updated, but pyRevit reload failed.\n\n{}".format(_safe_text(ex)),
            warn=True,
        )
        return False
