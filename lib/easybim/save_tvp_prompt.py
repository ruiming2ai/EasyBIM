...
def _handle_save_event(uiapp=None, event_args=None, action_label="saving"):
    ...
    try:
        ...
        LOGGER.warning("EasyBIM save TVP cleanup raised during %s: %s", action_label, ex)
        _show_alert(
            TITLE_ERROR,
            "EasyBIM could not remove Temporary View Properties before {}.\n\n{}".format(
                action_label,
                ex,
            ),
        )
    ...
    _show_alert(
        ...
        or "EasyBIM could not remove Temporary View Properties before {}.".format(
            action_label
        ),
    )
...
def _show_save_prompt(summary):
    ...
    try:
        ...
        if has_restore_work:
            ...
            dialog.MainContent = (
                "EasyBIM found Temporary View Properties work in this project.\n"
            ...
            dialog.ExpandedContent = (
                ...
                "Tracked EasyBIM temp-phase views to restore: {tracked}\n"
        ...
        LOGGER.warning("EasyBIM save prompt TaskDialog failed: %s", ex)
...
def _collect_summary(uiapp, doc):
    try:
        ...
        LOGGER.warning("EasyBIM save TVP prompt could not collect summary: %s", ex)
        summary = None
...
def _show_alert(title, message):
    ...
    print("[EasyBIM] {}: {}".format(title, message))
