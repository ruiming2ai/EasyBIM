# -*- coding: utf-8 -*-
"""Pure helpers for choosing loadable families - shared by Families Transfer
and Families Downgrade.

No Revit imports here: the option classes, the family-key scheme, the
sort/filter/merge helpers and the export filename helpers are plain Python
so the tests can drive them without an engine.  ``_safe_text`` is a local
copy on purpose (see ``easybim.compat``).
"""
